// app/challenge/[challengeId]/page.tsx
'use client';
...
export default function ChallengePlayPage() {
  ...
  const submit = () => {
    const total = answers.reduce((sum, a) => (a?.selected === a?.correct ? sum + 1 : sum), 0);
    setScore(total);
  ...
  Object.entries(q.options).map(([key, value]) => {
    const selected = answers[current]?.selected === key;
    return (
      <button
        key={key}
  ...
  <p className="text-red-500 font-semibold">⚠️ Options not available.</p>
)}
              ...
              <div className="flex flex-col lg:flex-row justify-between items-center gap-4">
                <button
                  onClick={toggleMark}
                  className={`px-4 py-2 rounded-xl shadow font-medium transition-all ${
