...
export async function POST(req) {
  ...
  if (!email) {
    return NextResponse.json(
      { success: false, message: "Unauthorized Access to server." },
      { status: 404 }
    );
  }
  ...
  if (user?.aiRemainingUses <= 0) {
    return NextResponse.json(
      { success: false, message: "AI usage limit reached. Come back next day." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    // Some models still wrap JSON in ``` blocks
